...
from utils import from_prediction_string_to_world_coords, from_camera_coords_to_image_coords
...
class TrainDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if torch.cuda.is_available():
            ...
            target = {'num_object': num_object.cuda(), 'pointmap': target_pointmap.cuda(), 'heatmap': target_heatmap.cuda(), 'local_offset': target_local_offset.cuda(), 'depth': target_depth.cuda() }
        else:
            image = image.contiguous()
            target = {'num_object': num_object, 'pointmap': target_pointmap, 'heatmap': target_heatmap, 'local_offset': target_local_offset, 'depth': target_depth}
