...
class StackedUNet(nn.Module):
    def __init__(self, head_list, num_stacks, in_channels, channel_list, batch_norm=False, head_relu=False, potential_map=False):
        ...
        for idx in range(num_stacks):
            net.append(UNet(head_list, channel_list, batch_norm=batch_norm, head_relu=head_relu, potential_map=potential_map))
...
class UNet(nn.Module):
    def __init__(self, head_list, channel_list, batch_norm=False, head_relu=False, potential_map=False):
        ...
        self.encoder = Encoder(channel_list, batch_norm=batch_norm)
        self.bottleneck_conv2d = BottleneckConv2d(channel_list[-1], channel_list[-1])
        self.decoder = Decoder(channel_list[::-1], batch_norm=batch_norm)
...
class Encoder(nn.Module):
    def __init__(self, channel_list, batch_norm=False):
        ...
        for idx in range(self.num_blocks):
            net.append(EncoderBlock(channel_list[idx], channel_list[idx+1], batch_norm=batch_norm))
...
class Decoder(nn.Module):
    def __init__(self, channel_list, batch_norm=False):
        ...
        for idx in range(self.num_blocks):
            net.append(DecoderBlock(channel_list[idx], channel_list[idx+1], batch_norm=batch_norm))
...
class EncoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, batch_norm=False, relu=True):
