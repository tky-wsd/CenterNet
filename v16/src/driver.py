...
class Trainer(object):
    def __init__(self, data_loader, model, optimizer, head_list, criterions, lambdas, args):
        ...
        if args.coeff_sigma_amplify == 0.0 and args.coeff_sigma!=args.coeff_sigma_max:
            raise ValueError("If args.coeff_sigma_amplify is 0, then you have to set args.coeff_sigma == args.coeff_sigma_max !")
        
        self.coeff_sigma_amplify = args.coeff_sigma_amplify
        self.coeff_sigma_max = args.coeff_sigma_max
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            if self.coeff_sigma_amplify > 0 and self.train_loader.dataset.coeff_sigma < self.coeff_sigma_max:
                print("Coeff sigma changed {}".format(self.train_loader.dataset.coeff_sigma), end='')
                self.train_loader.dataset.coeff_sigma = self.train_loader.dataset.coeff_sigma + args.coeff_sigma_amplify
                self.valid_loader.dataset.coeff_sigma = self.valid_loader.dataset.coeff_sigma + args.coeff_sigma_amplify
