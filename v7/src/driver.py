...
class Trainer(object):
    ...
    def run_one_epoch_train(self, epoch):
        ...
        for iteration, (train_images, target) in enumerate(self.train_loader):
            loss = self.run_one_batch(train_images, target, iteration, batch_size)
    ...
    def run_one_batch(self, train_images, target, iteration, batch_size):
        head_list = self.head_list
        batch_loss = 0
        batch_domain_loss = {}
        ...
        for (head, num_out_features, head_module) in head_list:
            print("({}): {}, ".format(head, batch_domain_loss[head].item()), end='')
            batch_loss = self.lambdas[head] * batch_domain_loss[head]
